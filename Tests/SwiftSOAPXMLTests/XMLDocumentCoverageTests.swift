...
final class XMLDocumentCoverageTests: XCTestCase {
    ...
    func test_initData_withURL_parsesSuccessfully() throws {
        ...
        let document = try SwiftSOAPXML.XMLDocument(
            data: Data(xml.utf8),
            sourceURL: sourceURL
        )
    ...
    func test_initURL_loadsDataAndParsesSuccessfully() throws {
        ...
        let document = try SwiftSOAPXML.XMLDocument(url: fileURL)
        let item = try document.xpathFirstNode("/root/item")
    ...
    func test_initData_withInvalidXML_throwsParseFailed() {
        ...
        XCTAssertThrowsError(try SwiftSOAPXML.XMLDocument(data: Data(invalidXML.utf8), logger: logger)) { error in
    ...
    func test_xpathNodes_invalidExpression_throws() throws {
        let document = try SwiftSOAPXML.XMLDocument(data: Data("<root/>".utf8))
    ...
    func test_xpath_noMatch_returnsNilAndEmptyArray() throws {
        let document = try SwiftSOAPXML.XMLDocument(data: Data("<root><item/></root>".utf8))
    ...
    func test_xpath_withInvalidNamespacePrefix_throws() throws {
        ...
        let document = try SwiftSOAPXML.XMLDocument(data: Data(xml.utf8))
    ...
    func test_serializedData_prettyPrinted_containsNewline() throws {
        let document = try SwiftSOAPXML.XMLDocument(data: Data("<root><child>v</child></root>".utf8))
    ...
    func test_nodeChildrenAndFirstChild_coverHelpers() throws {
        let document = try SwiftSOAPXML.XMLDocument(data: Data("<root><a>1</a><b id=\"x\"/></root>".utf8))
    ...
    func test_addNamespace_successAndLookup() throws {
        let document = try SwiftSOAPXML.XMLDocument(rootElementName: "Envelope")
        let root = try XCTUnwrap(document.rootElement())

        try root.addNamespace(SwiftSOAPXML.XMLNamespace(prefix: "soap", uri: "urn:soap"))
    ...
    func test_addNamespace_invalidConfiguration_throws() throws {
        ...
        XCTAssertThrowsError(try root.addNamespace(SwiftSOAPXML.XMLNamespace(prefix: "soap", uri: " "))) { error in
    ...
    func test_addChild_fromDifferentDocuments_throws() throws {
        let first = try SwiftSOAPXML.XMLDocument(rootElementName: "Root")
        let second = try SwiftSOAPXML.XMLDocument(rootElementName: "Root")
    ...
    func test_addChild_invalidOperation_throws() throws {
        let document = try SwiftSOAPXML.XMLDocument(rootElementName: "Root")
        let root = try XCTUnwrap(document.rootElement())
    ...
    func test_xmlNamespace_trimsEmptyPrefix() {
        let namespace = SwiftSOAPXML.XMLNamespace(prefix: "   ", uri: "urn:test")
    ...
    func test_builder_withDefaultNamespace_setsNamespaceURIWithoutPrefix() throws {
        let document = try SwiftSOAPXML.XMLDocument(
            ...
            rootNamespace: SwiftSOAPXML.XMLNamespace(uri: "urn:default")
        )
        let root = try XCTUnwrap(document.rootElement())
    ...
    func test_addNamespace_withDefaultNamespace_succeeds() throws {
        ...
        try root.addNamespace(SwiftSOAPXML.XMLNamespace(uri: "urn:default"))
