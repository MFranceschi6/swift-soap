...
//
// ## Mechanism B — Macros (Swift 5.9+ via SwiftSOAPXMLMacros)
//
...
///
/// - Note: If both `Foundation.XMLElement` and `SwiftSOAPXML.XMLElement` are in scope
///   (e.g. when importing `Foundation` and `SwiftSOAPXML`), qualify the wrapper
///   as `@SwiftSOAPXML.XMLElement` to avoid ambiguity.
///
/// ## Example
/// ```swift
/// @XMLCodable
...
///
/// - Note: If both `Foundation.XMLElement` and `SwiftSOAPXML.XMLElement` are in scope,
///   qualify the wrapper as `@SwiftSOAPXML.XMLElement` to avoid ambiguity. This is a
